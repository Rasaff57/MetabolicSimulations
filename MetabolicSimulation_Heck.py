...
def call_Mader(ATP_demand=np.repeat(250, 30*60), y0s=np.array([25, 350 / 60 / 18.2, 1.1, 1.1]),
               ...
               VolRel_b=0.45, Sc=25, Sa=7, M3=0.96, ATP_temp=5, Pi_temp=5, H_temp=10 ** -7, ATP_Ruhe=0):
    ...
    for n in range(0, len(ATP_demand)):
        ...
        x = solve_ivp(Model, t_span, y0s, args=(
            ATP_demand[n], VO2max, vlamax, Ks1, Ks2, Ks3, Kelox, KpyrO2, T,
...
CHEP_System, OxP, Lactate_Muscle, Lactate_Blood, ATP_store, pHm_store, AMP_store, ADP_store = call_Mader(ATP_demand= ATP_demand, VO2max=VO2max, vlamax=VLamax, VolRel_b=VolRel_b)
